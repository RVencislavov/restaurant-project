...
import static project.restaurant.interfaces.ColorAble.*;
...
public class Client {
    ...
    public void payBill(Waiter waiter) {
        ...
        if (wallet.subtract(bill).compareTo(new BigDecimal(0)) >= 0) {
            ...
            System.out.println(ANSI_RED + "Вие имате " + wallet + "лв. в портфейла.");
            bill = wallet;
            System.out.println(ANSI_RED + "НАПУСНЕТЕ НАШИЯ РЕСТОРАНТ ВЕДНАГА!!!" + ANSI_RESET + "\n");
        }
    }
